...
const QuickResponseScreen = () => {
  ...
  const initializeProgressTracking = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('🔄 [SCREEN] Loading current topic, progress, and total prompts...');
        await Promise.all([
          loadCurrentTopic(),
          loadUserProgress(),
          loadTotalPrompts()
  ...
  const loadPrompt = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await authenticatedFetch(API_ENDPOINTS.PROMPT(currentPromptId));
      const result = await response.json();
  ...
  const handleStartRecording = async () => {
    if (!currentPrompt || audioRecorder.state.isRecording || audioPlayer.state.isPlaying) {
  ...
  const processRecording = async (audioUri: string) => {
    ...
    try {
      ...
      console.log('📡 [SCREEN] Sending evaluation request to:', API_ENDPOINTS.EVALUATE_QUICK_RESPONSE);
      
      const evaluationResponse = await authenticatedFetch(API_ENDPOINTS.EVALUATE_QUICK_RESPONSE, {
        method: 'POST',
      ...
      console.log('✅ [SCREEN] Evaluation result received:', {
        ...
        exercise_completion: result.exercise_completion
      });
      ...
      if (result.success && result.evaluation && result.evaluation.is_correct) {
        if (result.exercise_completion?.exercise_completed) {
            ...
            setTimeout(() => {
              console.log('🔄 [SCREEN] Moving to next prompt after congratulations animation');
              setShowCongratulationsAnimation(false);
              moveToNextPrompt();
            }, 3000);
        }
  ...
  const moveToNextPrompt = () => {
    if (currentPromptId < totalPrompts) {
      ...
      setIsExerciseCompleted(true);
      setError('Congratulations! You have completed all Quick Response exercises!');
    }
  };
        ...
        <View style={styles.container}>
          ...
          >
            ...
            <View style={styles.titleContainer}>
              ...
              <Text style={styles.headerSubtitle}>Answer Questions Quickly</Text>
              
              {/* Question Counter */}
              <Text style={styles.questionCounter}>
                Question: {currentTopicId} of {totalPrompts}
              </Text>
            </View>
          </Animated.View>
              ...
              {isExerciseCompleted ? (
                <View style={styles.completedContainer}>
                  ...
                  <Text style={styles.completedText}>
                    Congratulations! You have successfully completed all Quick Response exercises.
                  </Text>
                ...
                >
                  <View style={styles.promptContainer}>
                    ...
                    <View style={styles.expectedAnswersContainer}>
                      ...
                      <Text style={styles.expectedAnswersText}>
        ...
        {showRetryAnimation && (
          <View style={styles.congratulationsOverlay}>
            ...
            <View style={styles.retryTextContainer}>
              ...
              <Text style={styles.retrySubtitle}>Answer the question</Text>
            </View>
          </View>
        )}
