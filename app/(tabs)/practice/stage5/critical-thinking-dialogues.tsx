...
const CriticalThinkingDialoguesScreen = () => {
    ...
    try {
      ...
      const response = await authenticatedFetch(API_ENDPOINTS.EVALUATE_CRITICAL_THINKING, {
        ...
        body: JSON.stringify({
          ...
          filename: `critical_thinking_${currentTopic.id}_${Date.now()}.m4a`,
          user_id: user.id,
      ...
      if (result.success) {
        ...
        if (result.error === 'no_speech_detected') {
          Alert.alert(
            'No Speech Detected',
            'Please speak clearly and provide a complete response. Try again.',
            [{ text: 'OK' }]
          );
        } else {
  ...
  const moveToNextTopic = () => {
    if (currentTopicId < totalTopics) {
      ...
      Alert.alert(
        'Congratulations! 🎉',
        'You\'ve completed all critical thinking dialogues!',
    ...
    >
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.container}>
          ...
          >
            <TouchableOpacity onPress={() => router.push({ pathname: '/practice/stage5' })} style={styles.backButton}>
            ...
            <View style={styles.titleContainer}>
              ...
              <Text style={styles.headerSubtitle}>Philosophical Dialogues</Text>
              
              {/* Topic Counter */}
              <Text style={styles.topicCounter}>
                Dialogue: {currentTopicId} of {totalTopics}
              </Text>
            </View>
          </Animated.View>
              ...
              {isExerciseCompleted ? (
                <View style={styles.completedContainer}>
                  ...
                  <Text style={styles.completedText}>
                    Congratulations! You have successfully completed all Critical Thinking dialogues.
                  </Text>
                  <Text style={styles.completedText}>Great job on your philosophical reasoning!</Text>
                </View>
              ) : isLoading ? (
                <View style={styles.loadingContainer}>
                  <Ionicons name="hourglass-outline" size={48} color="#58D68D" />
                  <Text style={styles.loadingText}>Loading philosophical topic...</Text>
                </View>
              ) : currentTopic ? (
                <ScrollView 
                ...
                >
                  <View style={styles.topicContainer}>
                    ...
                    <View style={styles.aiPositionContainer}>
                      <Text style={styles.aiPositionTitle}>AI's Position:</Text>
                      <Text style={styles.aiPositionText}>{currentTopic.ai_position}</Text>
                      <Text style={styles.aiPositionUrdu}>{currentTopic.ai_position_urdu}</Text>
                    </View>
                    
                    {/* Expected Keywords */}
                    <View style={styles.keywordsContainer}>
                      <Text style={styles.keywordsTitle}>Key Concepts to Address:</Text>
                    ...
                    <View style={styles.vocabularyContainer}>
                      <Text style={styles.vocabularyTitle}>Advanced Vocabulary:</Text>
                    ...
                    <Text style={styles.instructionText}>
                      Listen to the topic and present your philosophical argument
                    </Text>
                  </View>
                </ScrollView>
              ) : (
