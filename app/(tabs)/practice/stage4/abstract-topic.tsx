...
const AbstractTopicScreen = () => {
  ...
  const playTopicAudio = async () => {
    ...
    if (audioPlayer.state.isPlaying || isAudioLoading) {
      console.log("❌ [AUDIO] Audio already playing or loading");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.exercise_completion?.exercise_completed) {
          ...
          Alert.alert(
            'Congratulations!',
            'You have successfully completed all Abstract Topic exercises.',
