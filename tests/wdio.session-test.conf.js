...
// Simple session test configuration - just to verify BrowserStack connection
exports.config = {
  runner: 'local',
  specs: [
  ...
  capabilities: [{
    'bstack:options': {
      ...
      projectName: 'DIL-Session-Test',
      buildName: 'Session-Test-' + new Date().toISOString().split('T')[0],
  ...
  services: [
    ['browserstack', {
      ...
      testObservabilityOptions: {
        projectName: 'DIL-Session-Test',
        buildName: 'Session-Test-' + new Date().toISOString().split('T')[0],
        buildTag: ['session-test', 'android', 'app-automate']
      }
    }]
  ],
  framework: 'mocha',
